...
client.on('message', async msg => { 
  ...
  if (msg.guild = sunucuid) {
   ...
   if (owobott.includes(msg.author.id)) {
   ...
   if (owobott.includes(msg.author.id)) {
    ...
    if (command === `başlat`) {
      if (args[1] === `hepsi`) {
        msg.channel.send(`${hunt} kanalına \`13-31 saniye\` arasında __**owo hunt**__ yazılacak. \n${battle} kanalına \`13-31 saniye\` arasında __**owo battle**__ yazılacak. \n${praykanali} kanalına \`5-7 dakika\` arasında __**owo pray ${prayy}**__ yazılacak. \n\n**NOT:**  Komutlar çalışmıyorsa \`ayarlar.json\` dosyasındaki bilgilerin doğruluğunu kontrol ediniz.`)

        hunt.send(`owo hunt`)
        setInterval(() => {
          hunt.send(`owo hunt`)
           }, Math.floor(Math.random() * (31000 - 13000)) + 13000)
      ...
      if (args[1] === `hunt`) {
        ...
        if (msg.channel = hunt) {
          msg.channel.send(`**Hunt** komutu başlatıldı. Bu kanala \`13-31 saniye\` arasında __**owo hunt**__ yazılacak.`)
        }
      ...
      if (args[1] === `battle`) {
        if (msg.channel != battle) {
          msg.channel.send(`${battle} kanalına \`13-31 saniye\` arasında __**owo battle**__ yazılacak.  \n\n**NOT:** Komutlar çalışmıyorsa \`ayarlar.json\` dosyasındaki bilgilerin doğruluğunu kontrol ediniz.`)
        }
  
        if (msg.channel = battle) {
          msg.channel.send(`**Battle** komutu başlatıldı. Bu kanala \`13-31 saniye\` arasında __**owo battle**__ yazılacak.`)
        }
        ...
        if (args[1] != `başlat`) {
          if (args[1] != `hayvanlar`) {
            if (args[1] != `günlük`) {
              if (args[1] != `para`) {
                if (args[1] != `diğer`) {
                  msg.channel.send(`Lütfen \`başlat\` , \`hayvanlar\` , \`günlük\` , \`para\` , \`diğer\` kategorilerinden birini yazınız.`)
                }
              }
            }
          }
    ...
    if (command === `doğrula`) {
      if (!args[1]) {
        msg.channel.send(`Lütfen ${owobot} botuna özelden göndereceğiniz \`kelimeyi\` yazınız. \n\n**NOT:** Doğrulama yaparken doğrulama kodunu doğru girdiğinizden __**emin olun**__.`)
      }
    ...
    if (command === `send`) { 
      if (isNaN(args[1])) {
        msg.channel.send(`Lütfen geçerli bir \`rakam\` veya \`sayı\` girin.`)
      }
